...
/****************************************************************************
 * 
 * 知乎上的教程 https://zhuanlan.zhihu.com/p/573449685?utm_id=0 
...
namespace QFramework
{
	class Framework
	{
		...
		{
			ICommand iCommand;
			ICommand<A> iCommand_A;
			AbstractCommand abstractCommand;
			AbstractCommand<A> abstractCommand_A;
		}
	}
	#region Architecture
	...
	/// <summary>
	/// Model,System是HashSet
	/// <br/>Utility是Dic
	/// <br/>初始化Model在前，System在后
	/// </summary>
	...
	/// <summary>
	/// 01 表现层：ViewController层。IController接口，负责接收输入和状态变化时的表现，一般情况下，MonoBehaviour 均为表现层
	/// <para />可以获取System
	/// <br /> 可以获取Model
	/// <br /> 可以发送Command，IController 更改 ISystem、IModel 的状态必须用Command
	/// <br /> 可以监听Event
	/// <para /> ISystem、IModel状态发生变更后通知IController必须用事件或BindableProperty
	/// <br /> IController可以获取ISystem、IModel对象来进行数据查询
	/// <br /> 上层可以直接获取下层，下层不能获取上层对象
	/// <br /> 下层向上层通信用事件
	/// <br /> 上层向下层通信用方法调用（只是做查询，状态变更用Command），IController的交互逻辑为特别情况，只能用Command
	/// </summary>
	...
	/// <summary>
	/// 02 系统层：System层。ISystem接口，帮助IController承担一部分逻辑，在多个表现层共享的逻辑，比如计时系统、商城系统、成就系统等
	/// <para />可以获取System
	/// <br />可以获取Model
	/// <br />可以监听Event
	/// <br />可以发送Event
	/// <br />IController 更改 ISystem、IModel 的状态必须用Command
	/// <br />ISystem、IModel状态发生变更后通知IController必须用事件或BindableProperty
	/// <br />IController可以获取ISystem、IModel对象来进行数据查询
	/// </summary>
	...
	#region Model
	/// <summary>
	/// 03 数据层：Model层。IModel接口，负责数据的定义、数据的增删查改方法的提供
	/// <para />可以获取Utility
	/// <br />可以发送Event
	/// <br />IController 更改 ISystem、IModel 的状态必须用Command
	/// <br />ISystem、IModel状态发生变更后通知IController必须用事件或BindableProperty
	...
	/// <summary>
	/// 04 工具层：Utility层。
	/// <br/>IUtility接口，负责提供基础设施，比如存储方法、序列化方法、网络连接方法、蓝牙方法、SDK、框架继承等。
	/// <br/>啥都干不了，可以集成第三方库，或者封装API    
	/// </summary> 
	...
	/// <br />可以发送Event
	/// <br />可以发送Command
	/// <para />IController 更改 ISystem、IModel 的状态必须用Command
	/// <br />ISystem、IModel状态发生变更后通知IController必须用事件或BindableProperty
	/// <br />ICommand不能有状态
	/// <br /> Command 一般负责数据的 增 删 改，而 Query 负责数据的 查。
	/// </summary>  
    ...
    #region Query
    /// <see cref="AbstractQuery&lt;T&gt;>"/> for l list of all
	...
	/// <summary>
	///  首先 Controller 中的表现逻辑更多是接收到数据变更事件之后，对 Model 或者 System 进行查询，
	///  <br/>而查询的时候，有的时候需要组合查询，比如多个 Model 一起查询，查询的数据可能还需要转换一下，
	///  <br/>这种查询的代码量比较多。尤其是像模拟警用或者非常重数据的项目.
	///  <para/>Query 是一个可选的概念，如果游戏中数据的查询逻辑并不是很重的话，直接在 Controller 的表现逻辑里写就可以了，
	///  但是查询数据比较重，或者项目规模非常大的话，最好是用 Query 来承担查询的逻辑。
	///  <br />Command 一般负责数据的 增 删 改，而 Query 负责数据的 查。
	/// </summary> 
	...
	public interface IBindableProperty<T> : IReadonlyBindableProperty<T>
	{
        ...
        /// <summary>自定义,处理比如等级不大于最高等级之类</summary>
        //void SetValueWithLimit(Func<T,T> limitCb);
	}
	...
	public class BindableProperty<T> : IBindableProperty<T>
	{
        ...
        /// <summary>发生变化就会触发,但是不同的变化还是做switch之类,才能方便使用</summary>
        ...
        /// <summary>
        /// 没有任何变化的情况下，先返回一个当前的值，比较方便用于显示初始界面
        /// </summary>
