...
")]
    ...
    /// <summary>
    /// 01 约束了类型T
    /// <br/>02 防止多次回收，在Allocate和Recycle中进行拦截
    /// <br/>03自定义回收重置函数
    /// <br/>04（没看懂）不想让用户通过 SafeObjectPool 来 Allocate 和 Recycle 池对象了，那么 Allocate 和 Recycle 的控制权就要交给池对象来管理
    /// </summary>
    public class SafeObjectPool<T> : Pool<T>, ISingleton where T : IPoolable, new()
    {
        ...
        /// <summary>
        /// 不想让用户通过 SafeObjectPool 来 Allocate 和 Recycle 池对象了,
        /// <br/>那么 Allocate 和 Recycle 的控制权就要交给池对象来管理
        /// </summary>
